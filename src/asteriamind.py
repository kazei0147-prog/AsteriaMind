...
# 数值模式识别模板: 她自己发现幂律、线性关系等
def _numeric_pattern_condition(kg, entity, sigs):
    import re as _re
    by_pred = {}
    for r in kg.relations:
        if r.predicate in by_pred:
            by_pred[r.predicate].append(r)
        else:
            by_pred[r.predicate] = [r]
    for rels in by_pred.values():
        if len(rels) >= 3:
            nums_subj = [_re.findall(r'(\d+)', r.subject) for r in rels]
            nums_obj = [_re.findall(r'(\d+)', r.object) for r in rels]
            if all(ns and no for ns, no in zip(nums_subj, nums_obj)):
                return True
    return False

def _numeric_pattern_generate(kg, entity, sigs):
    """从共享 predicate 的关系中提取数值规律并预测"""
    import re as _re
    hyps = []
    by_pred = {}
    for r in kg.relations:
    ...
    for pred, rels in by_pred.items():
        ...
        pairs = []
        for r in rels:
            sn = _re.findall(r'(\d+)', r.subject)
            on = _re.findall(r'(\d+)', r.object)
            if sn and on:
                pairs.append((int(sn[0]), on[0], r))
        if len(pairs) < 3:
            continue
        # 排序
        pairs.sort()
        ...
        # 检查: subject 递增 1, object 中的数字是否线性?
        obj_nums = [_re.findall(r'(\d+)', o) for o in objs]
        if not all(on for on in obj_nums):
            continue
        obj_vals = [int(on[0]) for on in obj_nums]
        # 系数 = subject 数字 × obj 数字?
        ratios = [obj_vals[i] / nums[i] if nums[i] > 0 else 0 for i in range(len(nums))]
        if max(ratios) - min(ratios) < 0.01 and len(ratios) == len(nums) and ratios[0] > 0:
            # 发现线性关系! 预测下一个
            next_n = nums[-1] + 1
            predicted = f"{int(ratios[0]*next_n)}x^{next_n-1}"
            last_r = pairs[-1][2]
            hyps.append({
                "id": "H_NUMERIC",
                "label": f"数值规律: 每增加1, {pred}系数按{ratios[0]:.0f}倍增长, 预测 {last_r.subject}→{predicted}",
                "mechanism": "数值模式",
                "detail": f"从{len(pairs)}条观测发现{ratios[0]:.0f}倍线性关系",
                "confidence": 0.7,
                "prediction": f"预测 {last_r.subject.replace(str(nums[-1]), str(next_n))} {pred} {predicted}",
                "test": "计算验证",
                "discrimination": "区别于H1: 基于数值规律而非拓扑相似",
            })
    return hyps

tmpl_registry.register(HypothesisTemplate(
    id="H_NUMERIC", name="数值模式识别", mechanism="数值模式",
    condition_description="共享predicate且有数字规律时触发",
    complexity_cost=0.08, free_params=1, assumptions=2,
    condition_fn=_numeric_pattern_condition,
    generate_fn=_numeric_pattern_generate,
))
